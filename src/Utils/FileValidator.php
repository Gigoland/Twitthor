...
class FileValidator
{
     ...
     *
     * Updated from https://github.com/symfony/mime/blob/v5.1.8/MimeTypes.php#L1614 on 2020-11-18
     */
    private array $mapMime = [
    ...
    private array $mapMimeUpdate = [
        ...
        'm4a' => ['video/mp4', 'video/3gpp'],
        'mp3' => ['audio/x-mpeg-3', 'audio/mpeg3'],
        'otf' => ['application/vnd.ms-opentype'],
        'xls' => ['application/vnd.ms-office'],
        'csv' => ['text/plain', 'text/x-fortran'], // @todo php7 fix text/x-fortran
    ];

    /**
     * Denied extensions only
     */
    private array $onlyDeniedExt = [
        '7z',
        'c', 'cpp', 'csh', 'css',
        'exe',
        'go', 'gz',
        'h', 'hpp', 'hta', 'htaccess', 'htmls', 'htpasswd', 'htt', 'htx', // 'htm', 'html', - outiliser dans import maquettes
        'jar', 'jav', 'java', 'jcm', 'js', 'json', 'jsonld',
        'log',
        'mjs', 'mpkg',
        'php', 'phtml', 'py', 'pyc',
        'rar',
        'sh', 'sql', 'swf',
        'tar', 'tar.gz', 'torrent', 'tpl', 'twig',
        'xhtml', 'xul',
    ];
    ...
    {
        // Init all merge
        $this->mapMimeMerge = $this->getMapMime();
    }
    ...
    public function isValidUploadFile($file, array $allowedExtensions): bool
    {
        ...
        // Get mime type
        $mimeContentType = strtolower(mime_content_type($file['tmp_name']));

        // Empty mime type
        if (empty($mimeContentType)
         || $mimeContentType == 'inode/x-empty'
        ) {
            return false;
        }

        // Only ignored file extension (Method with logger)
        if ($this->isOnlyDeniedFile($extension, $mimeContentType, $file['name'])) {
            // File extension not allowed
            return false;
        }

        // Check mime type
        if (!in_array($mimeContentType, $this->mapMimeMerge[$extension])) {
            // File type not allowed
            return false;
        }

        // Check image exif type
        if (array_key_exists($extension, $this->mapImageExif)
         && !$this->isValidImageExif($file['tmp_name'], $extension)
        ) {
            // File type not allowed
            return false;
        }
    ...
    public function isValidStreamFile(string $url, array $allowedExtensions): bool
    {
        ...
        if (!empty($headers[0]) && !stripos($headers[0], "200")) {
            return false;
        }
        ...
        $mimeContentType = strtolower($finfo->buffer($buffer));
        ...
        // Only ignored file extension (Method with logger)
        if ($this->isOnlyDeniedFile($extension, $mimeContentType, $url)) {
            // File extension not allowed
            return false;
        }
        ...
        // Check image exif type
        if (array_key_exists($extension, $this->mapImageExif)
         && !$this->isValidImageExif($url, $extension)
        ) {
            // File type not allowed
            return false;
        }
    ...
    public function isValidLocalFile(string $location, array $allowedExtensions): bool
    {
        ...
        // Get mime type
        $mimeContentType = strtolower(mime_content_type($location));
        ...
        // Only ignored file extension (Method with logger)
        if ($this->isOnlyDeniedFile($extension, $mimeContentType, $location)) {
            // File extension not allowed
            return false;
        }
        ...
        // Check image exif type
        if (array_key_exists($extension, $this->mapImageExif)
         && !$this->isValidImageExif($location, $extension)
        ) {
            // File type not allowed
            return false;
        }
    ...
    {
        // Only ignored extensions
        if (in_array($extension, $this->onlyDeniedExt)) {
            // File extension not allowed
            return true;
        }

        // Check by mim type
        foreach ($this->onlyDeniedExt as $extensionDenied) {
            // Not have mime type in mapMimeMerge
            if (empty($this->mapMimeMerge[$extensionDenied])) {
                continue;
            }

            // Check mime type for denied extensions
            if (in_array($mimeContentType, $this->mapMimeMerge[$extensionDenied])
             && !in_array($mimeContentType, $this->mapMimeMerge[$extension])
            ) {
    ...
    private function isValidImageExif(string $filePath, string $extension): bool
    {
        ...
        // Check image exif type
        if (!in_array($exif, $this->mapImageExif[$extension])) {
            // File exif type not allowed
            return false;
        }
